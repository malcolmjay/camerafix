...
def _save_persistent_settings():
    payload = {
        "wifi_enabled": bool(globals().get("wifi_enabled", False)),
        "flip_mode": int(globals().get("flip_mode", 0)),
        "rangefinder_assist_enabled": bool(globals().get("rangefinder_assist_enabled", False)),
        "focus_peaking_enabled": bool(globals().get("focus_peaking_enabled", True)),
        "focus_mode_idx": int(globals().get("_focus_mode_idx", 0)),
        "ui_minimal_mode": bool(globals().get("_ui_minimal_mode", False)),
        "aspect_idx": int(globals().get("_aspect_idx", 0)),
        "film_idx": int(globals().get("_film_idx", 0)),
        "iso_idx": int(globals().get("_iso_idx", 0)),
        "shutter_idx": int(globals().get("current_shutter_idx", 0)),
        "exp_comp_idx": int(globals().get("_exp_comp_idx", 2)),
        "grid_overlay_idx": int(globals().get("_grid_overlay_idx", 0)),
        "focus_strength_idx": int(globals().get("_focus_strength_idx", 1)),
    }
    try:
        with _SETTINGS_LOCK:
...
def _set_cpu_low_power(enable=True):
    ...
    try:
        pols = _glob_module.glob("/sys/devices/system/cpu/cpufreq/policy*")
        for pol in pols:
...
_CPU_THERMAL_CAP_KHZ = 1200000  # 1.4 GHz cap (Pi 5 max is 2.4 GHz)
...
def _collect_power_stats(force_battery=False):
    ...
    try:
        ...
        lines.append(f"Storage: {used_gb:.1f} / {total_gb:.1f} GB ({free_gb:.1f} GB free)")
    except Exception:
        pass
...
# Focus peaking strength presets: blend_num / 100 = overlay opacity
_FOCUS_STRENGTH_OPTIONS = [
    {"label": "Low", "blend": 18},
    {"label": "Med", "blend": 35},
    {"label": "High", "blend": 55},
]
_FOCUS_STRENGTH_POPUP_IDX_OFFSET = 2000
_focus_strength_idx = _bounded_int(
    _PERSISTED_SETTINGS.get("focus_strength_idx", 1), 0, 0,
    len(_FOCUS_STRENGTH_OPTIONS) - 1)
...
def blit_add_with_shadow(dst, src, x, y, shadow_strength=0.5, blur_ksize=5):
    ...
    roi = ensure_channels(roi, tgt_ch)
    src_crop = ensure_channels(src_crop, tgt_ch)
    shadow = cv2.GaussianBlur(src_crop, (blur_ksize, blur_ksize), 0)
    shadow = cv2.convertScaleAbs(shadow, alpha=shadow_strength, beta=0)
    cv2.subtract(roi, shadow, dst=roi)
    cv2.add(roi, src_crop, dst=roi)
    dst[y1:y2, x1:x2] = roi
...
def apply_film_simulation_rgb(frame_rgb, key, skip_grain=False):
    if key == "none":
        return frame_rgb
    preset = _FILM_CURVE_DATA.get(key)
    if preset is None:
...
def apply_focus_peaking(frame_rgb, assist_frame=None):
    ...
    blend_num = _FOCUS_STRENGTH_OPTIONS[_focus_strength_idx]["blend"]
    ...
    if skip_frames > 0 and _focus_frame_counter % (skip_frames + 1) != 0:
        if _focus_last_mask is not None:
            ...
            if np.any(mask_idx):
                ...
                blended = (
                    (view.astype(np.uint16) * (100 - blend_num)
                     + _FOCUS_COLOR * blend_num)
                    // 100
                ).astype(np.uint8)
...
def _set_focus_strength(idx):
    """Set focus peaking overlay strength: 0=Low, 1=Med, 2=High."""
    global _focus_strength_idx, _focus_popup_visible, _focus_popup_rects, _focus_popup_bounds
    with _focus_button_lock:
        _focus_strength_idx = max(0, min(idx, len(_FOCUS_STRENGTH_OPTIONS) - 1))
        _focus_popup_visible = False
        _focus_popup_rects = []
        _focus_popup_bounds = None
    _record_activity(wake=True)
...
def _toggle_wifi():
    ...
    _debounced_save_settings()
    threading.Thread(target=_apply_wifi_state, args=(desired,), daemon=True).start()
...
def _render_focus_mode_popup(dst, bounds, fade_opacity, popup_visible,
                             mode_idx, strength_idx, margin=16):
    """Render combined popup: focus mode options + strength subsection (when peaking active)."""
    if not (popup_visible or fade_opacity > 0.01):
        return [], None
    ...
    corner_r = _POPUP_CORNER_RADIUS
    btn_corner_r = 6
    active_color = UI_ACCENT_FOCUS

    # --- Mode section layout (1 column) ---
    mode_labels = FOCUS_MODE_LABELS
    m_metrics = [_ui_measure_text(lbl, font_size) for lbl in mode_labels]
    m_max_w = max(m[0] for m in m_metrics)
    m_max_h = max(m[1] for m in m_metrics)
    m_btn_w = m_max_w + option_pad_x * 2
    m_btn_h = m_max_h + option_pad_y * 2
    mode_section_w = m_btn_w
    mode_section_h = len(mode_labels) * m_btn_h + (len(mode_labels) - 1) * gap_y

    # --- Strength section layout (only when peaking active) ---
    show_strength = (mode_idx == 1)
    if show_strength:
        strength_labels = [opt["label"] for opt in _FOCUS_STRENGTH_OPTIONS]
        s_metrics = [_ui_measure_text(lbl, font_size) for lbl in strength_labels]
        s_max_w = max(m[0] for m in s_metrics)
        s_max_h = max(m[1] for m in s_metrics)
        s_btn_w = s_max_w + option_pad_x * 2
        s_btn_h = s_max_h + option_pad_y * 2
        s_cols = len(strength_labels)
        strength_section_w = s_cols * s_btn_w + (s_cols - 1) * gap_x
        strength_section_h = s_btn_h

        section_label = "Strength"
        sec_tw, sec_th = _ui_measure_text(section_label, section_font_size)
        divider_gap = 12
        section_header_h = sec_th + 6
    else:
        strength_section_w = 0
        strength_section_h = 0
        divider_gap = 0
        section_header_h = 0

    # --- Panel dimensions ---
    content_w = max(mode_section_w, strength_section_w)
    panel_w = content_w + 2 * panel_pad_x
    panel_h = panel_pad_y + mode_section_h + panel_pad_y
    if show_strength:
        panel_h = (panel_pad_y + mode_section_h +
                   divider_gap + section_header_h + 4 +
                   strength_section_h + panel_pad_y)
    ...
    # Horizontal: right of button
    panel_x = max(margin, min(bounds[2] + gap, dst_w - panel_w - margin))
    panel_x2 = panel_x + panel_w
    panel_y2 = panel_y + panel_h
    ...
    # --- Draw mode options (1 column) ---
    mode_x_off = panel_pad_x + (content_w - mode_section_w) // 2
    mode_y_off = panel_pad_y
    for idx, lbl in enumerate(mode_labels):
        lx1 = mode_x_off
        ly1 = mode_y_off + idx * (m_btn_h + gap_y)
        lx2 = lx1 + m_btn_w
        ly2 = ly1 + m_btn_h
        btn_color = active_color if (idx == mode_idx) else (55, 55, 55)
    ...
    # --- Strength section (only when peaking is active) ---
    if show_strength:
        # Divider line
        div_y = mode_y_off + mode_section_h + divider_gap // 2
        ...
        # Strength buttons
        s_y_off = sec_label_y + section_header_h + 4
        s_x_off = panel_pad_x + (content_w - strength_section_w) // 2
        for idx, lbl in enumerate(strength_labels):
            lx1 = s_x_off + idx * (s_btn_w + gap_x)
            ly1 = s_y_off
            lx2 = lx1 + s_btn_w
            ly2 = ly1 + s_btn_h
            btn_color = active_color if (idx == strength_idx) else (55, 55, 55)
            _draw_rounded_rect(popup_img, (lx1, ly1), (lx2, ly2), btn_color, btn_corner_r)
            _draw_rounded_rect(popup_img, (lx1, ly1), (lx2 - 1, ly2 - 1), (140, 140, 140), btn_corner_r, 1)
            tw, th = _ui_measure_text(lbl, font_size)
            tx = lx1 + (s_btn_w - tw) // 2
            ty = ly1 + (s_btn_h - th) // 2
            text_items.append((lbl, tx, ty, (255, 255, 255)))
            # Rotated touch rect with offset
            rot_x1 = panel_x + (panel_w - lx2)
            rot_y1 = panel_y + (panel_h - ly2)
            rot_x2 = panel_x + (panel_w - lx1)
            rot_y2 = panel_y + (panel_h - ly1)
            popup_rects.append((_FOCUS_STRENGTH_POPUP_IDX_OFFSET + idx,
                                (rot_x1, rot_y1, rot_x2, rot_y2)))

        _ui_draw_text_batch(popup_img, text_items, font_size, outline_bgr=(0, 0, 0))
        # Section label at smaller font
...
def _draw_focus_controls(dst, top, right_edge, size=72, margin=16):
    ...
    with _focus_button_lock:
        ...
        strength_idx = _focus_strength_idx
    ...
    popup_rects, popup_bounds = _render_focus_mode_popup(
        dst, bounds, _focus_popup_fade, popup_visible,
        active_idx, strength_idx, margin=margin,
    )
...
def _render_brenizer_help_overlay(canvas):
    ...
    blit_add(canvas, help_block, hx, hy)
...
def _render_brenizer_complete_screen(canvas):
    ...
    if time.monotonic() < _brenizer_merge_toast_until:
        ...
        blit_add(canvas, toast_block, toast_x, toast_y)
...
def _draw_multishot_controls(dst, top, right_edge, size=72, margin=16):
    """Draw the multishot button (Double Exp / Brenizer) with popup menu.

    When Brenizer mode is active, shows the grid icon instead of the
    double-exposure overlapping-frames icon.
    """
    ...
    bounds = _icon_button_bounds(dst.shape, top, right_edge, size, margin)
    is_active = double_on or double_waiting or bren_active
    ...
    # Determine active index: -1 if none, 0 if DE, 1 if B5, 2 if B6, 3 if B9
    active_idx = -1
    if double_on:
        active_idx = 0
...
def _on_touch(event, x, y, flags, param):
    ...
    if focus_popup_visible:
        for idx, rect in focus_popup_rects:
            if _point_in_rect(x, y, rect):
                if idx >= _FOCUS_STRENGTH_POPUP_IDX_OFFSET:
                    _set_focus_strength(idx - _FOCUS_STRENGTH_POPUP_IDX_OFFSET)
                else:
                    _set_focus_mode(idx)
                return
    ...
    if _point_in_rect(x, y, _double_exposure_button_rect):
        ...
        return
    # Handle multishot popup item taps (Double Exp / Brenizer 6 / Brenizer 9)
    ...
    for idx, line in enumerate(measured_lines):
        ...
        for seg in line["segments"]:
            ...
            draw.text((x + seg["x_off"], seg_y + seg["y_off"]),
                      seg["text"], fill=(255, 255, 255), font=seg["font"])
            x += seg["w"]
        y += line["height"]
...
def make_rotated_text_block(
    ...
    # Each segment becomes a (text, font_scale_or_None) tuple so that
    # segment boundaries are preserved and differently-styled inputs
    # never collide.
    key_lines = []
    for raw_line in lines:
        if isinstance(raw_line, (list, tuple)) and not isinstance(raw_line, str):
            ...
            for seg in raw_line:
                if isinstance(seg, dict):
                    segs.append((seg.get("text", ""), seg.get("font_scale", None)))
                elif isinstance(seg, (list, tuple)) and seg:
                    segs.append((str(seg[0]), None))
                else:
                    segs.append((str(seg), None))
            key_lines.append(tuple(segs))
        else:
            key_lines.append(((str(raw_line), None),))
...
def _pace_frame():
    ...
    if sleep_t > 0:
        ...
        _next_frame_t = now
_main_frame_counter = 0
_cached_hist_img = None
_cached_batt_rotated = None
_cached_batt_key = None

# Low storage threshold: warn at startup when free space < 1 GB
_LOW_STORAGE_THRESHOLD_BYTES = 1 * 1024 * 1024 * 1024
_low_storage_warning_until = 0.0

# --- Startup low-storage check ---
try:
    _startup_disk = shutil.disk_usage(LOCAL_PICTURES_DIR)
    if _startup_disk.free < _LOW_STORAGE_THRESHOLD_BYTES:
        _low_storage_warning_until = time.monotonic() + 5.0
except Exception:
    pass

try:
    while not _shutdown_flag.is_set():
        _main_frame_counter += 1
        ...
        if _gain > 1.01:
            frame = cv2.convertScaleAbs(frame, alpha=_gain, beta=0)
        ...
        # Only prepare assist frame when needed (focus peaking or rangefinder)
        if (focus_peaking_enabled or rangefinder_assist_enabled) and not _backlight_dimmed:
        ...
        if rangefinder_assist_enabled and not _backlight_dimmed:
        ...
        # Skip when dimmed (idle) — user can't see the effect at low brightness.
        # Throttle grain noise to every 2nd frame in preview only (saves CPU;
        # imperceptible at 20 FPS). Capture/save path always gets full grain.
        if film_key != "none" and not _backlight_dimmed:
            _skip_preview_grain = (_main_frame_counter % 2 != 0)
            scaled = apply_film_simulation_rgb(scaled, film_key, skip_grain=_skip_preview_grain)

        if focus_peaking_enabled and not _backlight_dimmed:
        ...
        # Throttle histogram: every 2nd frame normal, every 3rd when dimmed
        _hist_skip = 3 if _backlight_dimmed else 2
        if _main_frame_counter % _hist_skip == 0 or _cached_hist_img is None:
            _cached_hist_img = draw_histogram(gray_for_hist, height=BAR_H, width=hist_w, rotated=True)
        hist_img = _cached_hist_img
        left_margin = 18
        hist_x = left_margin
        blit_add(canvas, hist_img, hist_x, bar_y)

        # Battery icon in bottom row, left-aligned after histogram
        # Cache the rendered battery icon — only re-render when pct or ac_state changes
        with _batt_lock:
            pct = batt_percent_cached
        ac_state = _read_pld_state()
        _batt_key = (pct, ac_state)
        if _batt_key != _cached_batt_key or _cached_batt_rotated is None:
            batt_iw, batt_ih = 30, 13
            batt_nub_w = 3
            bolt_size = 14 if ac_state == 1 else 0
            # Reuse pre-allocated buffers instead of np.zeros every frame
            ...
            _cached_batt_rotated = cv2.rotate(batt_buf, cv2.ROTATE_180)
            _cached_batt_key = _batt_key
        batt_buf = _cached_batt_rotated
        batt_x = SCREEN_W - 18 - batt_buf.shape[1]
        fixed_bar_y = SCREEN_H - BAR_H
        batt_y = fixed_bar_y + (BAR_H - batt_buf.shape[0]) // 2
        blit_add(canvas, batt_buf, batt_x, batt_y)
        ...
        # Skip SS/ISO and zoom text when Brenizer mode provides its own status overlay
        if not _brenizer_active:
            shutter_line = make_rotated_text_block(
                [[{"text": "SS ", "font_scale": 1.2}, f"{shutter_label}   ", {"text": "ISO ", "font_scale": 1.0}, {"text": iso_display, "font_scale": 1.4}]],
                font_scale=1.8,
                thickness=2,
                rotate_180=True,
                max_h=BAR_H + 8,
            ...
            zoom_line = make_rotated_text_block(
                [[f"{zoom_factor:.1f}x   IC: {image_count}{ev_suffix}{double_suffix}"]],
                font_scale=1.0,
                thickness=1,
                rotate_180=True,
                max_h=BAR_H - 10,
        ...
        if show_wifi_info:
            ...
            blit_add(canvas, wifi_block, wx, wy)
        ...
        if _shutter_osd_fade > 0.01 and _shutter_osd_text:
            ...
            # Render text into a tight buffer, then rotate 180°
            ...
            # Additive blend scaled by fade opacity
            ...
            if _osd_x1 < _osd_x2 and _osd_y1 < _osd_y2:
                ...
                _osd_src = _osd_buf[_osd_sy1:_osd_sy1 + (_osd_y2 - _osd_y1),
                                    _osd_sx1:_osd_sx1 + (_osd_x2 - _osd_x1)]
                ...
                _osd_shd = cv2.GaussianBlur(_osd_src, (5, 5), 0)
                _osd_shd = cv2.convertScaleAbs(_osd_shd, alpha=0.5, beta=0)
                cv2.subtract(_osd_roi, _osd_shd, dst=_osd_roi)
                cv2.add(_osd_roi, _osd_src, dst=_osd_roi)

        # --- Low storage warning OSD ---
        if time.monotonic() < _low_storage_warning_until:
            _ls_line1 = "Low Storage"
            _ls_line2 = "Delete files to continue"
            _ls_font1 = max(24, int(SCREEN_H * 0.07))
            _ls_font2 = max(16, int(SCREEN_H * 0.04))
            _ls_w1, _ls_h1 = _ui_measure_text(_ls_line1, _ls_font1)
            _ls_w2, _ls_h2 = _ui_measure_text(_ls_line2, _ls_font2)
            _ls_bw = max(_ls_w1, _ls_w2) + 40
            _ls_bh = _ls_h1 + _ls_h2 + 30
            _ls_buf = np.zeros((_ls_bh, _ls_bw, 3), dtype=np.uint8)
            _ui_draw_text(_ls_buf, _ls_line1, (_ls_bw - _ls_w1) // 2, 8, _ls_font1,
                          UI_ACCENT_DANGER, outline_bgr=(0, 0, 0))
            _ui_draw_text(_ls_buf, _ls_line2, (_ls_bw - _ls_w2) // 2, 8 + _ls_h1 + 10, _ls_font2,
                          UI_ACCENT_SYSTEM, outline_bgr=(0, 0, 0))
            _ls_buf = cv2.rotate(_ls_buf, cv2.ROTATE_180)
            _ls_x = (SCREEN_W - _ls_bw) // 2
            _ls_y = (SCREEN_H - _ls_bh) // 2
            _ls_x1 = max(0, _ls_x)
            _ls_y1 = max(0, _ls_y)
            _ls_x2 = min(SCREEN_W, _ls_x + _ls_bw)
            _ls_y2 = min(SCREEN_H, _ls_y + _ls_bh)
            if _ls_x1 < _ls_x2 and _ls_y1 < _ls_y2:
                _ls_bg = canvas[_ls_y1:_ls_y2, _ls_x1:_ls_x2]
                cv2.convertScaleAbs(_ls_bg, alpha=0.4, beta=0, dst=_ls_bg)
                _ls_sx1 = _ls_x1 - _ls_x
                _ls_sy1 = _ls_y1 - _ls_y
                _ls_src = _ls_buf[_ls_sy1:_ls_sy1 + (_ls_y2 - _ls_y1),
                                  _ls_sx1:_ls_sx1 + (_ls_x2 - _ls_x1)]
                cv2.add(_ls_bg, _ls_src, dst=_ls_bg)
